...
abstract public class Unit extends Thing implements Observer
{
    ...
    public boolean collision(double e, double d, int w, int h)
    {
        if(getDone())
        {
            return false;
        ...
        if (d + h/2 > getY()  - getImage().getHeight(null)/2
                && d - h/2 < getY()  + getImage().getHeight(null)/2
                && e + w/2 > getX()  - getImage().getWidth(null)/2
                && e - w/2 < getX()  + getImage().getWidth(null)/2) 
        {
            return true;
        } else
